...
@Service
public class AuthorTimeout implements JavaDelegate {

    private final UserService userService;
    private final MailService mailService;

    public AuthorTimeout(UserService userService, MailService mailService) {
    ...
    public void execute(DelegateExecution delegateExecution) throws Exception {
        final String username = (String) delegateExecution.getVariable("authorId");
        final User author = userService.findByUsername(username);

        mailService.sendMail(author.getEmail(), "Paper rejected", "Paper rejected because of technical reasons.");
    }
}
